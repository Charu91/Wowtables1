...
use Mail;
use Mailchimp;
use WowTables\Http\Models\Frontend\GiftCardsModel;
...
class AdminGiftCardsController extends Controller{
    ...
    public function checkGiftCard(){
        ...
        $dataPost['CardId'] = Input::get('CardId');
            ...
            $result = DB::select('SELECT * FROM gift_cards WHERE gift_card_id = "'.$dataPost['CardId'].'"');
            if(count($result)>=1){
                $data['exists'] = true;
    ...
    public function addGiftCard(){
      //  $password = str_random(6);
		...
		if($Gift_card_id =='0'){
		$gift_id = DB::table('gift_cards')->insertGetId([
            'gift_card_id' => $GiftId,
            'buyer' => $Buyer_name,
	...
	$update_gift_id=DB::table('gift_cards')->where('id', '=', $Gift_card_id)
    ...
    public function detailGiftCard(){
        ...
        if(isset($GIft_CardId) && $GIft_CardId != "" && $GIft_CardId > 0){
           $gift_detail = DB::table('gift_cards')->where('id','=',$GIft_CardId)->first();
