...
class Password {
        ...
        static $arrWebsitePasswordConfirm=array(
                                            'token' => 'required|passwordResetToken',
                                            'password' => 'required',
                                        );
        ...
        public static function requestWebsitePassword($data) {
            //creating a new token
    ...
    Mail::raw($mailbody, function($message) use ($data)
    	{
        	...
        	$message->to($data['email'])->subject('Forgot Password Request on WowTables.com');
    	});
    }

    static public function sendWebsiteMail($data){
    //echo "<pre>"; print_r($data); die;
    /*$mailbody =  "User - ".$data['email'].";<br/>Hi {$data['userName']},<br>
        We have received a forgot password request from you. If you have sent the request, please use the link below to set a new password.
        <br /><a href='".URL::to('forgotPassword')."/".$data['user_id']."/".$data['randString']."'>".URL::to('forgotPassword')."/".$data['user_id']."/".$data['randString']. "</a><br />
        If you have not sent the request then you do not need to do anything.<br />
        <br>Thanks & Regards<br>
        The WowTables Team";*/

    Mail::send('site.pages.website_forgot_password',[
        'data'=> $data
    ...
    static public function saveWebsitePasswordRequest($data) {
